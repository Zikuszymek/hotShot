...
public class UniversalRefresh {
    ...
    public static final String POBIERANIE_INFORMACJI = "HotShot: Trwa pobieranie informacji";
    ...
    public static void RefreshAllIfNoLongedRefreshed(final Activity activity){
        ...
        if(lastCheck == 0 || ((currentTime - lastCheck) > HOUR)) {
            Log.d("TEST","HotShots need to be refreshed");
            AsyncTaskHotShot hotShotsAsync = new AsyncTaskHotShot(new Runnable() {
                @Override
                public void run() {
                    UniversalRefresh.RefreshCategoriesAndWebPages(activity.getBaseContext());
                    UniversalRefresh.RefreshAllIfPossible(activity);
                }
            },activity.getBaseContext(),true);
            ShowToasWithMessage(activity.getBaseContext(),POBIERANIE_INFORMACJI);
            hotShotsAsync.execute();
        }
         else {Log.d("TEST","HotShots up to date");}
    }
